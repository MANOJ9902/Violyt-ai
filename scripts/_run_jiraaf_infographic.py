"""Run Jiraaf LinkedIn infographic; auto-approve blueprint; print image URL."""
...
PROMPT = (
    "Create a clean LinkedIn infographic: Why bond markets connect investors to "
    "real-economy financing. Use 2 key stats and 2 short distinct supporting insights. "
    "No filler phrases. Short CTA Explore More."
)
...
async def main() -> int:
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        print(f"[{_now()}] START infographic", flush=True)
        resp = await client.post(f"{BASE}/api/v1/pipeline/run", json=body)
        resp.raise_for_status()
        run_id = resp.json()["run_id"]
        print(f"[{_now()}] run_id={run_id}", flush=True)
        (OUT / "_jiraaf_infographic_run_id.txt").write_text(run_id, encoding="utf-8")
        ...
        for _ in range(200):
            ...
            if status == "awaiting_blueprint_approval" and not approved:
                (OUT / "_jiraaf_infographic_phase1.json").write_text(
            ...
            if status in {"complete", "failed", "cancelled"}:
                (OUT / "_jiraaf_infographic_phase2.json").write_text(
                    json.dumps(last, indent=2, default=str), encoding="utf-8"
                )
                if imgs:
                    (OUT / "_jiraaf_infographic_image_urls.txt").write_text(
