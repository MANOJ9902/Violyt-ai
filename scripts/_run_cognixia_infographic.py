"""Run Cognixia LinkedIn infographic with gpt-image-2; auto-approve; print image URL."""
...
BRAND_ID = "f5072038-e3b5-40de-8d49-c074fe5015d6"
OUT = Path(r"C:\Users\Lenovo\ai10\Violyt-ai\storage")
OUT.mkdir(parents=True, exist_ok=True)

PROMPT = (
    "Create an infographic on how organizations are benefitting from using AI "
    "in cybersecurity. Use 4 distinct supporting insights with short stats. "
    "Insight-led for enterprise LinkedIn. Short CTA Explore More."
)
...
async def main() -> int:
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        print(f"[{_now()}] START infographic (gpt-image-2)", flush=True)
        resp = await client.post(f"{BASE}/api/v1/pipeline/run", json=body)
        resp.raise_for_status()
        run_id = resp.json()["run_id"]
        print(f"[{_now()}] run_id={run_id}", flush=True)
        (OUT / "_cognixia_infographic_run_id.txt").write_text(run_id, encoding="utf-8")
        ...
        for _ in range(240):
            ...
            if status == "awaiting_blueprint_approval" and not approved:
                (OUT / "_cognixia_infographic_phase1.json").write_text(
            ...
            if status in {"complete", "failed", "cancelled"}:
                (OUT / "_cognixia_infographic_phase2.json").write_text(
                    json.dumps(last, indent=2, default=str), encoding="utf-8"
                )
                if imgs:
                    (OUT / "_cognixia_infographic_image_urls.txt").write_text(
