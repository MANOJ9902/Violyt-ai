...
class RendererService:
    ...
    def _fit_text_block(
        ...
        while font_size >= min_size:
            font = self._font(font_size, family_hint=family_hint, weight=weight)
        ...
        fallback_font = self._font(min_size, family_hint=family_hint, weight=weight)
            ...
            return [
                {
                    ...
                    "solution_statement": metadata.get("solution_statement") or "",
                    "customer_quote": metadata.get("customer_quote") or "",
                    ...
                    "slide_copy": metadata.get("slide_copy", []) if isinstance(metadata.get("slide_copy"), list) else [],
                    "hashtags": metadata.get("hashtags", []) if isinstance(metadata.get("hashtags"), list) else [],
                    "claim_safety_notes": metadata.get("claim_safety_notes", []) if isinstance(metadata.get("claim_safety_notes"), list) else [],
    ...
    def _render_page(
        ...
        if payload.blueprint.layout_type == "infographic" and not template_rendered and not prefer_blueprint_zones:
            try:
                ...
                logger.error("renderer.infographic_page_failed: %s", exc, exc_info=True)
                raise
        # This guard handles missing or invalid input early so the main workflow can stay straightforward.
        if (
